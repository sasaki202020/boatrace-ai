...
Day 2: current feature matrix を 3 つの可用性に分類した。
...
判定ルール:
- 当日取得可能: 当日のレースカード・直前計測で取得する列
- 前日まで確定: 出走表/過去成績/事前集計で前日までに確定する列
- 事後情報の恐れあり: 結果確定後の情報、または future leakage を疑う列

注: current feature matrix には `finish_position` や `target_win` のような教師列は含めていない。もし training-only 列を別途見るなら、これは別表で扱うのが安全。
...
| feature_name | source_file | source_column | when_available | leakage_risk | missing_rate | notes |
| --- | --- | --- | --- | --- | ---: | --- |
| `boat_no` | `src/features/build_features.py` | `historical_races.csv.boat_no (alias lane)` | ingest / pre-race | low | 0.000000 | 号艇。active set の基礎列。lane と同義の運用キー。 |
| `avg_st` | `src/features/build_features.py` | `historical_races.csv.avg_st` | pre-race (history-derived) | low | 0.179000 | 平均ST。trainable では補完対象だが、可用性としては事前確定の履歴系。 |
...
- `exhibition_time`, `exhibition_time_rank`, `start_timing` は当日取得可能に分類した。
- `avg_st` は履歴由来の事前確定列として分類した。trainable では補完されるが、availability 上は pre-race 扱いに寄せる。
- `recent*`, `course*`, `lane_win_rate_prior`, `win_rate_venue`, `rank_trend` は前日まで確定に分類した。
- current feature matrix では、事後情報の恐れありに該当する feature は見当たらなかった。
