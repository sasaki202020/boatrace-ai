"""
レーサー期別成績 (fan*.LZH) ダウンロードスクリプト

fan ファイルは月単位 (fan{YYMM}.lzh) で提供される。
全期間分をカバーするには、期別更新月 (4月/10月) を含む月範囲を指定する。

使い方:
  py scripts/download_fanbook.py --start 202401 --end 202612
  py scripts/download_fanbook.py --start 202401 --end 202612 --dry-run
"""
import argparse
import os
import sys
...
from dl_common import (
    build_url, month_range, download_one,
...
def main():
    parser = argparse.ArgumentParser(description="レーサー期別成績 (fan*.LZH) ダウンロード")
    parser.add_argument("--start", required=True, help="開始月 (YYYYMM)")
    parser.add_argument("--end", required=True, help="終了月 (YYYYMM)")
    ...
    for dt in month_range(args.start, args.end):
    ...
    logger.info("=== Fanbook Download Summary ===")
