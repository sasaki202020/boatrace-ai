...
from src.feature_forward_v1.source_policy import policy_from_manifest
...
def personal_valid(approval):
 try:
  policy=policy_from_manifest(approval);gates=policy.evaluate()
...
def main(argv=None):
 ...
 personal=personal_valid(approval) if approval.get("schemaVersion")==2 else None
 basic=((approval.get("schemaVersion")==1 and approval.get("collectionEnabled") is True) or (personal is not None and approval.get("manualIngestAllowed") is True)) and bool(approval.get("allowedSourceTypes"))
 ...
 preflight=preflight_manual_inbox(inbox=args.inbox,approval=approval,store_root=args.store,now=datetime.now(timezone.utc),paths=files,manual_enabled=True)
 preflight_payload=json_safe(preflight)
 if preflight["status"]=="EMPTY_MANUAL_INBOX":
  payload={**base,"status":"WAITING_FOR_APPROVED_INPUT","manualIngest":preflight_payload}
  if personal is not None:payload.update({"usageMode":"PERSONAL_RESEARCH_ONLY","personalResearchStatus":"ALLOWED_WITH_RESTRICTIONS","automatedFetchStatus":"PERSONAL_COLLECTION_MANUAL_ONLY"})
  write_json(args.status,payload);append_log(args.log or args.status.with_name("operations_log.jsonl"),payload);print(json.dumps(payload));return 0
 if preflight["status"]!="MANUAL_INGEST_PREFLIGHT_READY":
  payload={**base,"status":"MANUAL_INGEST_PREFLIGHT_BLOCKED","manualIngest":preflight_payload,"reason":"manual_ingest_preflight_failed"}
  if personal is not None:payload.update({"usageMode":"PERSONAL_RESEARCH_ONLY","personalResearchStatus":"ALLOWED_WITH_RESTRICTIONS","automatedFetchStatus":"PERSONAL_COLLECTION_MANUAL_ONLY"})
  write_json(args.status,payload);append_log(args.log or args.status.with_name("operations_log.jsonl"),payload);print(json.dumps(payload));return 2
 ...
 results=[];attempted=0
 with collector.lock():
  for path in preflight["readyPaths"]:
 ...
 captured=sum(r.status=="CAPTURED" for r in results);stored=sum(r.status!="ALREADY_CAPTURED" and collector.store.existing(r.snapshot_id) is not None for r in results);payload={**base,"status":"BLOCKED_INTEGRITY" if not valid else "COLLECTION_COMPLETED","processedFiles":len(results),"capturedFiles":captured,"featureStoreWrites":stored,"rejectedFiles":sum(r.status in {"REJECTED","QUARANTINED_SCHEMA_DRIFT"} for r in results),"quality":quality,"manualIngest":preflight_payload}
 if personal is not None:
  payload.update({"usageMode":"PERSONAL_RESEARCH_ONLY","personalResearchStatus":"ALLOWED_WITH_RESTRICTIONS","automatedFetchStatus":"PERSONAL_AUTOMATED_COLLECTION_READY" if personal[1].automated_fetch_gate=="READY" else "PERSONAL_COLLECTION_MANUAL_ONLY"})
