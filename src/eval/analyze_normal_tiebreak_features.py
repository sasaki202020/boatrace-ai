...
def main():
    parser = argparse.ArgumentParser(description="Analyze normal-race winner vs predicted-top1 feature diffs.")
    ...
    parser.add_argument("--out-diff-csv", default="reports/tiebreak_feature_diff.csv")
    parser.add_argument("--out-json", default="reports/tiebreak_feature_summary.json")
    ...
    feature_cols = [
        ...
        if c not in exclude_cols and pd.api.types.is_numeric_dtype(proba_df[c])
    ]

    # Normalize base score columns
    ...
    for _, row in normal_df.iterrows():
        ...
        rec = {
            ...
            "pred_top1_lane": str(top1_row["lane"]),
        }
        for feat in feature_cols:
            w_val = winner_row.get(feat, None)
            t_val = top1_row.get(feat, None)
            if pd.notna(w_val) and pd.notna(t_val):
                w_val = float(w_val)
                t_val = float(t_val)
                rec[f"{feat}_winner"] = round(w_val, 6)
                rec[f"{feat}_top1"] = round(t_val, 6)
                rec[f"{feat}_diff"] = round(w_val - t_val, 6)
    ...
    summary = {
        "normal_races_input": int(len(normal_df)),
    ...
    for feat in feature_cols:
        ...
        summary["features"][feat] = stat

    summary["tiebreak_candidates"] = [
        feat
        for feat, stat in summary["features"].items()
        if stat["winner_higher_rate"] >= args.winner_higher_threshold
    ]
