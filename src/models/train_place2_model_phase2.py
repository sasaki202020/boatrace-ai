...
"""Train the Phase 2 place2 conditional model."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Train the Phase 2 place2 conditional model")
    ...
    try:
        ...
        print(f"Saved model bundle: {result['bundle_path']}")
        print(f"Saved report JSON: {args.report_json}")
        print(f"Saved report CSV: {result['csv_path']}")
        print(f"Saved split manifest: {result['split_manifest_path']}")
        return 0
    except Exception as exc:
        logger.exception("Phase 2 training failed")
        print(f"[error] {exc}")
        return 1
